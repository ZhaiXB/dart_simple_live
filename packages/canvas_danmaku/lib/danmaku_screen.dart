...
class _DanmakuScreenState<T> extends State<DanmakuScreen<T>>
  ...
  void _stepFrame([int? elapsedMs]) {
    if (!_running || !mounted) return;
    final tickVal = elapsedMs ?? DateTime.now().millisecondsSinceEpoch;
    _notifier.value = tickVal;
    if (_time++ > 10) {
      _time = 0;
      _lazyTick(tickVal);
    }
  ...
  void _tick(Duration elapsed) {
    if (_option.externalDrive) return;
  ...
  void _handleAddDanmaku(
    ...
    if (_running && added) {
      if (!_option.externalDrive && !_ticker.isActive) {
        _ticker.start();
      }
    }
  }
